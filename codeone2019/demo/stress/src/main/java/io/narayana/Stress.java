...
public class Stress extends AbstractVerticle {
    ...
    public static void main(String[] args) throws InterruptedException, ExecutionException {
        ...
        if (opts.getStringOption("help", null) != null) {
            System.out.printf("syntax: Stress [parallelism=20] [requests=10] [port=8080] [url=/api/trip/Odeon/ABC]%n");
            return;
        }
        ...
        System.out.printf("%d out of %d requests failed in %d ms%n",
                failCnt.get(), target, duration / 1000000);
    ...
    private void invokeService(
            HttpClient client, int port, String host, String url, boolean verbose) {
        client.post(port, host, url)
                .exceptionHandler(e -> {
                    incrFailCount(); System.out.printf("request failed: %s%n",
                ...
                .handler(h -> {
                    ...
                    if (verbose) System.out.printf("%d and %d of %d %s%n",
                            passCnt, failCnt, targetCnt.getCount(), h.toString());
                })
                .end();
    }
